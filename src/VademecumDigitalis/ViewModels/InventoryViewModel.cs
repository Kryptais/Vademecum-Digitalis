...
namespace VademecumDigitalis.ViewModels
{
    public partial class InventoryViewModel : ObservableObject
    {
        ...
        private async Task DeleteContainer(InventoryContainer container)
        {
            ...
            string? delOption = await _dialogService.DisplayActionSheet($"Löschen: {container.Name}", "Abbrechen", "Löschen & Inhalt vernichten", "Löschen & Inhalt verschieben");
            ...
            if (delOption == "Löschen & Inhalt verschieben")
            {
                ...
                if (targets.Length == 0)
                {
                    await _dialogService.DisplayAlert("Fehler", "Kein Ziel-Container verfügbar.", "OK");
                    return;
                }

                string? targetName = await _dialogService.DisplayActionSheet("Ziel wählen", "Abbrechen", null, targets);
        ...
        private async Task EditContainer(InventoryContainer container)
        {
            ...
            string? subAction = await _dialogService.DisplayActionSheet($"Bearbeiten: {container.Name}", "Abbrechen", null, "Name ändern", "Details ändern", "Münzgewicht an/aus");
